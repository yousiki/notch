...
final class VolumeManager: NSObject, ObservableObject {
    ...
    @MainActor func increase(stepDivisor: Float = 1.0) {
        ...
        setAbsolute(target)
        CapsuleViewCoordinator.shared.toggleSneakPeek(status: true, kind: "volume", value: CGFloat(target))
    }
    ...
    @MainActor func toggleMuteAction() {
        ...
        toggleMuteInternal()
        CapsuleViewCoordinator.shared.toggleSneakPeek(status: true, kind: "volume", value: CGFloat(willBeMuted ? 0 : resultingVolume))
    }
