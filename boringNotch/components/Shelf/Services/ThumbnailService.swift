...
extension QLThumbnailGenerator: @unchecked Sendable {}

actor ThumbnailService {
    nonisolated(unsafe) static let shared = ThumbnailService()
            ...
            return await withCheckedContinuation { (continuation: CheckedContinuation<NSImage?, Never>) in
                thumbnailGenerator.generateBestRepresentation(for: request) { representation, error in
                    if let rep = representation {
                        ...
                        if let err = error {
                            NSLog(
                                "⚠️ ThumbnailService: thumbnail error for \(scopedURL.path): \(err.localizedDescription)"
                            )
                        }
