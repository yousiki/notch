...
final class NowPlayingController: ObservableObject, MediaControllerProtocol, @unchecked Sendable {
    ...
    init?() {
        do {
            mediaRemote = try MediaRemoteClient()
        } catch {
            AppLogger.log("Failed to load MediaRemote: \(error)", category: .error)
            return nil
        }
    ...
    func toggleShuffle() async {
        ...
        mediaRemote.setShuffleMode(playbackState.isShuffled ? 1 : 3)
    ...
    func toggleRepeat() async {
        ...
        mediaRemote.setRepeatMode(newRepeatMode)
    }
    ...
    private func setupNowPlayingObserver() async {
        ...
        guard
            ...
            AppLogger.log("Could not find mediaremote-adapter.pl script or framework path", category: .error)
            return
        }
        ...
        do {
            ...
            AppLogger.log("Failed to launch mediaremote-adapter.pl: \(error)", category: .error)
        }
    }
    ...
    private func handleAdapterUpdate(_ update: NowPlayingUpdate) async {
        ...
        if let elapsedTime = payload.elapsedTime {
            ...
            if payload.playing == false {
                ...
                newPlaybackState.currentTime =
                    self.playbackState.currentTime + (self.playbackState.playbackRate * timeSinceLastUpdate)
            } else {
...
actor JSONLinesPipeHandler {
    ...
    func readJSONLines<T: Decodable & Sendable>(as type: T.Type, onLine: @escaping @Sendable (T) async -> Void)
        async
    {
        do {
            ...
            AppLogger.log("Error processing JSON stream: \(error)", category: .error)
        }
    }

    private func processLines<T: Decodable & Sendable>(as type: T.Type, onLine: @escaping @Sendable (T) async -> Void)
        async throws
    {
        while true {
    ...
    func close() async {
        do {
            ...
            AppLogger.log("Error closing pipe handler: \(error)", category: .error)
        }
    }
}
