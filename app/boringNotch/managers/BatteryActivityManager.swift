...
class BatteryActivityManager: @unchecked Sendable {

    nonisolated(unsafe) static let shared = BatteryActivityManager()

    var onBatteryLevelChange: (@Sendable (Float) -> Void)?
    var onMaxCapacityChange: (@Sendable (Float) -> Void)?
    var onPowerModeChange: (@Sendable (Bool) -> Void)?
    var onPowerSourceChange: (@Sendable (Bool) -> Void)?
    var onChargingChange: (@Sendable (Bool) -> Void)?
    var onTimeToFullChargeChange: (@Sendable (Int) -> Void)?

    private var batterySource: CFRunLoopSource?
    private var observers: [@Sendable (BatteryEvent) -> Void] = []
