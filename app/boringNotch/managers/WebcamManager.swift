...
class WebcamManager: NSObject, ObservableObject, @unchecked Sendable {
    nonisolated(unsafe) static let shared = WebcamManager()
    ...
    private func setupCaptureSession(completion: @escaping @Sendable (Bool) -> Void) {
        sessionQueue.async { [weak self] in
            ...
            do {
                ...
                guard session.canAddInput(videoInput) else {
                    throw NSError(
                        domain: "BoringNotch.WebcamManager", code: -1,
                ...
                let previewSession = UnsafeTransfer(value: session)
                DispatchQueue.main.async {
                    self.cameraAvailable = true
                    let previewLayer = AVCaptureVideoPreviewLayer(session: previewSession.value)
